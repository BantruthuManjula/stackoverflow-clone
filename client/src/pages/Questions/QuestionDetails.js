...
const user=useSelector((state)=>state.currentUserReducer)
 const handlePostAns=(e,answerLength)=>
 {
  e.preventDefault()
   if(user===null)
   {
       alert('login or signup to answer a quesion')
       Navigate('/Auth')
   }
   else{
        if(Answer===''){
           ...
           <>
           {
            ...
            <section className='question-details-container'>
              ...
              <div className='question-details-container-2'>
               <div className='question-votes'>
                <img src={upvote} alt='upvote' width='18' className='icon' onClick={handleUpVote}/>
                <p>{question.upVote.length-question.downVote.length}</p>
                <img src={downvote} alt="downvote" width='18' className='icon' onClick={handleDownVote}/>
               </div>
               <div style={{width:'100%'}}>
               <p className='quesion-body'>{question.questionBody}</p>
            ...
            </section>
            {
              question.answer.length!==0 &&(
                <section>
                  ...
                  <DisplayAnswer key={question._id }
            ...
            <section className='post-ans-container'>
              ...
              <form onSubmit={(e)=>{handlePostAns(e,question.answer.length)}}>
                <textarea name="" id="" cols="30" rows="10" onChange={(e)=>setAnswer(e.target.value)}/ ><br/>
                <input type="submit" className='post-ans-btn' value='post your answer'/>
              </form>
              <p>Browse other Question tagged
                {
                  question.questionTags.map((tag)=>(
                    <Link to='/' key={tag} className='ans-tags'>
                      {""}
                      {tag} {''}
                    </Link>
